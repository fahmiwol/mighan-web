...
| Field | Nilai terakhir (2026-04-30 — AGENT-20260430-004) |
|--------|-----------------------------|
...
| **Prioritas disepakati** | **Sprint 1 — Room Viewer Infrastructure:** ✅ `/room/[roomId]` page live (auth + fetch room state + 2D overview + 3D iframe mode). ✅ `docs/SAAS_PRODUCT_VISION.md` committed. **Deploy:** mighan-web PM2 id 25 online VPS. **Arsitektur final:** `ops.mighan.com` = global engine inventory (admin-only, asset server) → user room viewer pakai iframe ke `/room-viewer` path di ops.mighan.com (belum ready — ops DNS ke Cloudflare origin unknown). **Pending Sprint 2:** (1) ops.mighan.com `/room-viewer` page — engine Three.js dalam user mode; (2) nginx wildcard `*.mighan.com` + `room.mighan.com` → Next.js; (3) `/dashboard/rooms/{id}` room management page; (4) NPC hire flow + marketplace stub; (5) Register flow E2E test; (6) Ixonomic wallet integration (stub → real). |
...
| **Tool status** | SSH VPS via `~/.ssh/sidix_session_key` → `root@72.62.125.6` ✅ jalan. Bash tool ✅ jalan. PM2 mighan-web id 25. |
| **Blokir** | `ops.mighan.com` DNS ke Cloudflare (origin unknown) → `/room-viewer` path belum bisa di-deploy. Marketplace + NPC hire belum ada backend. Ixonomic wallet masih stub. `ADMIN_PASSWORD=mighan2026` hardcoded — security risk. |
| **Jangan commit** | `server/settings.json`, `server/.data/`, `.env`, secret, session cookies. |
| **Files kunci Mighan-Web** | `app/room/[roomId]/page.tsx` (room viewer), `app/dashboard/rooms/page.tsx` (rooms list), `docs/SAAS_PRODUCT_VISION.md` (North Star + roadmap), `components/PortalNav.tsx`, `components/AuthProvider.tsx`. |
| **Peta proyek** | `docs/SAAS_PRODUCT_VISION.md` = North Star dokumen utama untuk SaaS vision + sprint roadmap. `docs/PROJECT_CARTOGRAPHY.md` = peta repo Mighan-3D. |
